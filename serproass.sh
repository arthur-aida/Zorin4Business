...
if [ "$(id -u)" != "0" ] ; then
	zenity   --warning --text="Solicite ao Administrador que execute, como root, executar no terninal (bash serproass.sh) para instal�-lo " --width=650 --height=150 --timeout=15
	exit
fi
...
if [ ! -n "$pacote" ]; then
	if [ "$(id -u)" = "0" ]; then
		...
		zenity   --warning --text="Na barra de localiza��o do menu, digite: Assinador Serpro " --width=450 --height=100 --timeout=15
fi
